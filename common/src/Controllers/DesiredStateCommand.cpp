...
void DesiredStateCommand<T>::convertToStateCommands() {
  ...
  data.stateDes(6) = 0;//deadband(leftAnalogStick[1], minVelX, maxVelX);  // forward linear velocity
  data.stateDes(7) = deadband(rightAnalogStick[1], minVelY, maxVelY);  // lateral linear velocity
  ...
  data.stateDes(3) = deadband(leftAnalogStick[1],minRoll,maxRoll);//0.0; // Roll
  data.stateDes(4) = deadband(leftAnalogStick[0], minPitch, maxPitch);  // Pitch
  data.stateDes(5) = deadband(rightAnalogStick[0], -0.4, 0.6); //Yaw fatto da me
