...
int Simulation::kick(double dist_wx, double dist_wy,double dist_wz) 
{
...
//     dist_wz = wz[j];
    ...
    FBModelState<double> state = getRobotState();

    kickVelocity << dist_wx,dist_wy,0.0,0.0,0.0,0.0;
...
void Simulation::step(double dt, double dtLowLevelControl,
  ...
  // Automatic kick to the robot along wx and wy. Decomment to test it
  
  double start;
    
  if(_robotParams.roa_test == 1 && firstTime == true && _robotParams.control_mode == 2)
  {
     start = _currentRealTime;
     firstTime = false;
  }
  
  else
  {
      double stop = _currentRealTime;
        
      double duration = (stop - start);
      
    if(_robotParams.roa_test == 1 && _robotParams.control_mode == 2 && (int)duration % 15 == 0)
    {
        
        if(end_kick != 1)
        {
            end_kick = kick(dist_wx,dist_wy,dist_wz);
        }
...
//                 simRate);
        _currentRealTime = freeRunTimer.getSeconds();
        updateGraphics();
      }
